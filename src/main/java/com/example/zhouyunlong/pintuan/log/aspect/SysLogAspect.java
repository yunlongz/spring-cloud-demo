package com.example.zhouyunlong.pintuan.log.aspect;
...
import com.example.zhouyunlong.pintuan.log.event.SysLogEvent;
import com.example.zhouyunlong.pintuan.log.util.LogUtil;
 ...
 * 3 * @Date: 2019/11/16 15:39
 * 4 * @Version 1.0
 * 5 * ①切面注解得到请求数据 -> ②发布监听事件 -> ③异步监听日志入库
 */
@Slf4j
@Aspect
@Component
public class SysLogAspect {
     ...
     */
    @Pointcut("@annotation(com.example.zhouyunlong.pintuan.log.annotation.SysLog)")
...
//        sysLog.setUserName(securityUser.getUsername());
    ...
    public void doAfterReturning(Object ret) {
        ...
        // 处理完请求，返回内容
        if(ret != null){
            CommonResult commonResult = Convert.convert(CommonResult.class, ret);
            if (commonResult.getCode() == 0){
                // 正常返回
                sysLog.setOpType(1);
            } else {
                sysLog.setOpType(2);
                sysLog.setExDetail(commonResult.getMsg());
            }
            sysLog.setStatusCode(commonResult.getCode());
            sysLog.setStatusMessage(commonResult.getMsg());
        }
        // 发布事件
